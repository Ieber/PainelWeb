 ...
 */
 ...
 //Função que vocaliza as informações da senha
 function vocalizarTexto(text) {
    ...
    .controller('PainelCtrl', function($scope, $http) {
        ...
        $scope.chamar = function() {
            if (PainelWeb.started && $scope.senhas.length > 0) {
				...
				//vocalizarTexto(senha.nomeCliente + " " + senha.local);
			
				//Vocaliza o nome do cliente, o local e o numero do local.
				vocalizarTexto(senha.nomeCliente)
				.then(() => {
					vocalizarTexto(senha.local)
						.then(() => {
							vocalizarTexto(senha.numeroLocal)
								.then(() => {
									vocalizarTexto(' ')
									})
								})
